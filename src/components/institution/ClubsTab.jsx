...
export const ClubsTab = ({ institutionName, clubs, addClub, dark, addToast }) => {
  const d = dark;
    ...
    <div className="space-y-5">
      ...
      {instClubs.length > 0 && (
        <div className="space-y-2">
          {instClubs.map(club => (
            <div key={club.id} className={`flex items-center gap-3 p-3.5 rounded-xl border ${d ? 'bg-surface-900 border-surface-800' : 'bg-white border-surface-200'}`}>
      ...
      <AnimatePresence>
        {showAdd && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
            ...
            {/* Name */}
            <input type="text" placeholder="e.g., Football Club, Debate Society" value={name} onChange={(e) => setName(e.target.value)}
              className={`w-full p-3 border rounded-xl text-sm outline-none focus:border-primary-500 transition ${d ? 'bg-surface-800 border-surface-700 text-white' : 'bg-surface-50 border-surface-200 text-surface-900'}`} autoFocus />
