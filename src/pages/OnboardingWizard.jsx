...
import { EDU, EDU_PICKER, STREAM_OF, INSTITUTIONS } from '../constants/education';
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <div className="flex-1 px-6 py-4 overflow-y-auto pb-36">
        ...
        {/* Step 1: Education Level — flat 7-bucket picker. Each bucket's
            description lists what it covers (HSC + A-Level + Alim etc.) so
            students from any stream self-identify in one click. */}
        {step === 1 && (
          <div className="flex flex-col gap-2">
            {EDU_PICKER.map((id) => {
              const m = EDU[id];
              if (!m) return null;
              const selected = form.educationLevel === id;
              return (
                <button key={id} onClick={() => upd("educationLevel", id)}
                  className={`flex items-center gap-3 p-3.5 rounded-2xl border-2 text-left transition ${selected ? "border-indigo-500 bg-indigo-50" : "border-slate-100 bg-white hover:border-slate-200"}`}>
                  <div className={`w-11 h-11 ${m.bgColor} rounded-xl flex items-center justify-center text-xl shrink-0`}>{m.icon}</div>
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-bold ${selected ? "text-indigo-700" : "text-slate-700"}`}>{m.label}</p>
                    <p className="text-xs text-slate-500 mt-0.5">{m.desc}</p>
                  </div>
                  {selected && <span className="text-indigo-600 font-bold text-lg shrink-0">✓</span>}
                </button>
              );
            })}
          </div>
