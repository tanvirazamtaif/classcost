...
const HousingEntryPage = () => {
  ...
  const handleAddItem = (typeId) => {
    const type = HOUSING_TYPES.find(t => t.id === typeId);
    if (!type) return;
    haptics.light();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      addToast('Housing payment saved', 'success');
      navigate('dashboard');
    } catch (e) {
      console.error(e);
      haptics.error();
  ...
  const availableTypes = HOUSING_TYPES.filter(t => !items.some(i => i.subType === t.id));
    ...
    <motion.div {...pageTransition} className={`min-h-screen ${d ? 'bg-surface-950' : 'bg-surface-50'}`}>
      ...
      <div className="sticky top-0 z-30 bg-white/95 dark:bg-surface-950/95 backdrop-blur-sm border-b border-surface-200 dark:border-surface-800">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div>
            <h1 className={`text-lg font-semibold ${d ? 'text-white' : 'text-surface-900'}`}>Housing Payment</h1>
            <p className="text-xs text-surface-500">🏠 Add housing expenses</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-md mx-auto p-4 pb-24">
          ...
          <motion.div {...fadeInUp} className="space-y-3">
            {items.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-xs text-surface-500">Tap the button below to add housing expenses</p>
              </div>
            ) : (
              items.map((item) => {
                const type = HOUSING_TYPES.find(t => t.id === item.subType);
        ...
        <div className="mt-6">
          <GButton fullWidth size="lg" onClick={handleSave} loading={saving} disabled={mode === 'bulk' ? !bulkAmount : totalIndividual === 0}>
            Save housing payment
          </GButton>
        </div>
      </div>

      <BottomSheet isOpen={showAddItem} onClose={() => setShowAddItem(false)} title="Add housing item">
...
export default HousingEntryPage;
