...
import { EDUCATION_TYPES } from '../types/payment';
...
const EducationEntryPage = () => {
  ...
  const handleAddItem = (typeId) => {
    const type = EDUCATION_TYPES.find(t => t.id === typeId);
    if (!type) return;
    haptics.light();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      addToast('Education payment saved', 'success');
      navigate('dashboard');
    } catch (e) {
      console.error(e);
      haptics.error();
  ...
  const availableTypes = EDUCATION_TYPES.filter(t => !items.some(i => i.subType === t.id));
    ...
    <motion.div {...pageTransition} className={`min-h-screen ${d ? 'bg-surface-950' : 'bg-surface-50'}`}>
      ...
      <div className="sticky top-0 z-30 bg-white/95 dark:bg-surface-950/95 backdrop-blur-sm border-b border-surface-200 dark:border-surface-800">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div>
            <h1 className={`text-lg font-semibold ${d ? 'text-white' : 'text-surface-900'}`}>Education Payment</h1>
            <p className="text-xs text-surface-500">🎓 Add education expenses</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-md mx-auto p-4 pb-24">
          ...
          <motion.div {...fadeInUp} className="space-y-3">
            {items.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-xs text-surface-500">Tap the button below to add education expenses</p>
              </div>
            ) : (
              items.map((item, i) => {
                const type = EDUCATION_TYPES.find(t => t.id === item.subType);
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="p-1 rounded-full hover:bg-danger-50 dark:hover:bg-danger-500/10 text-danger-500"
                          >
        ...
        <div className="mt-6">
          <GButton
            ...
            onClick={handleSave}
            loading={saving}
            disabled={mode === 'bulk' ? !bulkAmount : totalIndividual === 0}
          >
            Save education payment
          </GButton>
        </div>
      </div>

      {/* Add Item Sheet */}
      <BottomSheet isOpen={showAddItem} onClose={() => setShowAddItem(false)} title="Add education item">
          ...
          {availableTypes.map(type => (
            <button
              ...
              className={`w-full flex items-center gap-3 p-4 rounded-xl transition ${
                d ? 'hover:bg-surface-800' : 'hover:bg-surface-50'
              }`}
            >
...
export default EducationEntryPage;
