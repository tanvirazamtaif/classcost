...
import { ArrowLeft, Calendar, Bell, Calculator, Plus, Check, AlertCircle, Info, Camera } from 'lucide-react';
...
export const EducationFeeFormPage = () => {
  ...
  // Receipt scanner state (mobile only)
  ...
  return (
    <motion.div {...pageTransition} className={`min-h-screen pb-28 ${d ? 'bg-surface-950' : 'bg-surface-50'}`}>
      ...
      {/* Receipt Scanner (mobile only, Google Lens style) */}
      {isMobile && (
        <ReceiptScanner
          isOpen={showScanner}
          ...
          onCreditDetect={usePerCredit ? (data) => {
            setRegularRate(String(data.rate));
            setRegularCredits(String(data.credits));
            addToast(`Detected: ৳${data.rate}/credit × ${data.credits}`, 'success');
          } : undefined}
        />
      )}
    </motion.div>
